...
class UserRepository:
    ...
    async def get_user_by_id(self, user_id: str, org_id: str) -> User:
        result =await self.db.execute(select(User).where(User.id == user_id and User.organization_id == org_id))
    ...
    async def update_user(self, user_id: str, org_id: str, user_data) -> User:
        user = await self.get_user_by_id(user_id, org_id)
        if user:
